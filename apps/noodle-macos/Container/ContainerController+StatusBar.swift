...
extension ContainerController {
    func setupStatusItem() {
        ...
        if let button = statusItem.button {
            if let icon = NSImage(named: "StatusBarIcon") {
                ...
                // Fallback if the asset catalog is missing.
                button.title = "🦙"
            }
        }
        ...
        let setTrustEnvItem = NSMenuItem(
            title: "Set Trust Env Vars (launchctl setenv)",
            action: #selector(setTrustEnvVarsAction(_:)),
            keyEquivalent: ""
        )
        setTrustEnvItem.target = self
        menu.addItem(setTrustEnvItem)
    ...
    func setStatus(status: NEVPNStatus, detail: String?) {
        ...
        if let button = statusItem?.button {
            // Image (if present) was set in setupStatusItem; leave it.
            // Only refresh the tooltip here so the status text stays
            // current.
    ...
    func disconnectDebugHint(_ error: NSError) -> String? {
        ...
        case ("NEVPNConnectionErrorDomainPlugin", 7), ("NEVPNConnectionErrorDomain", 12):
            return
                "inspect `~/Library/Logs/DiagnosticReports/NoodleExtension*.ips` and `log show --last 5m --style compact --predicate 'process == \"NoodleExtension\" OR subsystem == \"com.noodleproxy.macos\"'`"
        case ("NEVPNConnectionErrorDomain", 14):
            return
