...
extension ContainerController {
    ...
    func installMITMCA() {
        ...
        Task { [weak self] in
            do {
                let reply = try await client.call(NoodleTproxyInstallRootCA.self)
    ...
    func clearCA() {
        ...
        Task { [weak self] in
            do {
                let reply = try await client.call(NoodleTproxyUninstallRootCA.self)
                await MainActor.run {
    ...
    func rotateMITMCAAndApply() {
        ...
        Task { [weak self] in
            do {
                let reply = try await client.call(NoodleTproxyRotateRootCA.self)
    ...
    private func applyInstallReply(_ reply: NoodleTproxyRootCaReply) {
    ...
    private func applyUninstallReply(_ reply: NoodleTproxyRootCaReply) {
        guard reply.ok else {
    ...
    private func applyRotateReply(_ reply: NoodleTproxyRotateRootCA.Reply) {
        guard reply.ok else {
            let message = reply.error ?? "unknown sysext error"
    ...
    private func certificateFromReply(
        _ reply: NoodleTproxyRootCaReply,
        op: String
    ) -> SecCertificate? {
        guard reply.ok else {
            let message = reply.error ?? "unknown sysext error"
    ...
    /// Provider-INDEPENDENT uninstall purge. `clearCA()` cannot be
    /// used by the uninstall pipeline: it guards on
    /// `isProviderActive()` and needs XPC to a running provider,
    /// but the pipeline stops the proxy in step 1 — so `clearCA()`
    /// no-ops and the CA is never removed. This enumerates EVERY
    /// noodle CA in the System keychain (repeated installs /
    /// rotations accumulate several), drops admin trust and
    /// deletes each, and LOOPS until the keychain reports none —
    /// "delete until they're all gone" — then wipes the
    /// SE-encrypted secret blobs.
    func purgeAllNoodleCAsForUninstall() {
        var keychainRef: SecKeychain?
        let openStatus = SecKeychainOpen(
            "/Library/Keychains/System.keychain", &keychainRef)
        guard openStatus == errSecSuccess, let keychain = keychainRef else {
            logErrorText(
                "purgeAllNoodleCAs: SecKeychainOpen failed (OSStatus \(openStatus)); still wiping secrets"
            )
            wipeStoredCASecretsLocally()
            return
        }
        ...
        while pass < maxPasses {
            ...
            guard status == errSecSuccess,
                ...
                logErrorText(
                    "purgeAllNoodleCAs: query failed (OSStatus \(status))")
                break
            }
            let noodleCerts = certs.filter { Self.isNoodleCA($0) }
            if noodleCerts.isEmpty { break }
            for cert in noodleCerts {
                let trust = SecTrustSettingsRemoveTrustSettings(
                    cert, Self.trustDomain)
                if trust != errSecSuccess, trust != errSecItemNotFound {
                    logErrorText(
                        "purgeAllNoodleCAs: trust remove failed (OSStatus \(trust))"
                    )
                }
                ...
                if del != errSecSuccess, del != errSecItemNotFound {
                    logErrorText(
                        "purgeAllNoodleCAs: cert delete failed (OSStatus \(del))"
                    )
                }
            }
            totalRemoved += noodleCerts.count
            log(
                "purgeAllNoodleCAs: pass \(pass) removed \(noodleCerts.count) noodle CA cert(s)"
            )
        }
        if pass >= maxPasses {
            logErrorText(
                "purgeAllNoodleCAs: hit \(maxPasses)-pass cap; some certs may remain"
            )
        }
        log("purgeAllNoodleCAs: removed \(totalRemoved) noodle CA cert(s) total")
        wipeStoredCASecretsLocally()
    }

    /// True when `cert` is a noodle MITM root CA. Pulls CN and
    /// subject-summary strings out of the certificate and delegates
    /// the matching logic to `isNoodleCertificateAuthority` (a pure
    /// free function in `IsNoodleCA.swift` — covered by unit tests
    /// without needing a real `SecCertificate`).
    private static func isNoodleCA(_ cert: SecCertificate) -> Bool {
        var cn: CFString?
        let cnString: String? =
            SecCertificateCopyCommonName(cert, &cn) == errSecSuccess
            ? cn as String?
            : nil
        let summaryString =
            SecCertificateCopySubjectSummary(cert) as String?
        return isNoodleCertificateAuthority(
            commonName: cnString,
            subjectSummary: summaryString
        )
    }
}
