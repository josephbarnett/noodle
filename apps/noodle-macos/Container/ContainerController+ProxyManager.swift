...
extension ContainerController {
    ...
    /// Load every saved `NETransparentProxyManager` profile on the
    /// machine and remove all of them. Used by the Uninstall flow to
    /// clear the entry shown under System Settings → Network →
    /// VPN & Filters.
    func loadAndRemoveAllProxyManagers(completion: @escaping (Bool) -> Void) {
        NETransparentProxyManager.loadAllFromPreferences { managers, error in
            if let error {
                self.logError("loadAllFromPreferences error", error)
                completion(false)
            ...
            guard let managers, !managers.isEmpty else {
                self.log("no transparent-proxy managers to remove")
                completion(true)
                return
            }
            self.log("removing \(managers.count) transparent-proxy manager(s) from preferences")
            self.removeManagersFromPreferences(managers, completion: completion)
        }
    }
}
