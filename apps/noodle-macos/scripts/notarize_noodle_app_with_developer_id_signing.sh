...
BUILD_SCRIPT="$SCRIPT_DIR/build_noodle_app_with_developer_id_signing.sh"
DERIVED_DATA_PATH="${NOODLE_TPROXY_DERIVED_DATA_PATH:-$ROOT_DIR/.xcode-derived/noodle-app-dist}"
APP_PATH="${NOODLE_TPROXY_DIST_APP_PATH:-$DERIVED_DATA_PATH/Build/Products/Release/Noodle.app}"
NOTARY_DIR="${NOODLE_TPROXY_NOTARY_DIR:-$DERIVED_DATA_PATH/notarization}"
ZIP_PATH="$NOTARY_DIR/Noodle.zip"
...
KEYCHAIN_PROFILE="${NOODLE_TPROXY_NOTARY_KEYCHAIN_PROFILE:-}"
APPLE_ID="${NOODLE_TPROXY_NOTARY_APPLE_ID:-}"
PASSWORD="${NOODLE_TPROXY_NOTARY_PASSWORD:-}"
...
Provide one of:
  NOODLE_TPROXY_NOTARY_KEYCHAIN_PROFILE=<stored notarytool profile>
  NOODLE_TPROXY_NOTARY_APPLE_ID=<apple id>
  NOODLE_TPROXY_NOTARY_PASSWORD=<app-specific password>
Recommended setup:
  xcrun notarytool store-credentials noodle-tproxy-notary     --apple-id <apple-id> --team-id $TEAM_ID --password <app-specific-password>
Then run with:
  NOODLE_TPROXY_NOTARY_KEYCHAIN_PROFILE=noodle-tproxy-notary
EOF
  exit 1
fi
