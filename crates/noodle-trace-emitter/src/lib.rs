//! Offline OTLP trace emitter (story 061, ADR 057 — dev-only) — reconstruction
//! library.
//!
//! The reconstruction half lives here so it is reusable: the `main` binary
//! POSTs the rows to a collector, and the story-060 regression test asserts the
//! span tree the rows produce. Both exercise the **real** ADR 052 §5 frame-tree
//! detector ([`noodle_adapters::marking`]) and the **real** story-060 row →
//! span path ([`noodle_shipper`]) — never a reimplementation.
//!
//! It reads each `*_request.json` in a capture in order, reconstructs the
//! round-trip's marks, and maps it to a [`RollupsRow`]. The capture carries no
//! wall-clock timestamps, so the clock is **synthesised** from each entry's
//! `_link.global_seq` (1 s spacing, fixed 800 ms latency) — enough for spans to
//! nest and order correctly; it is not real latency.
...
use noodle_shipper::RollupsRow;

/// The committed capture the emitter and the story-060 regression test replay:
/// one turn, ROOT + three parallel sub-agents, 12 round-trips.
pub const DEFAULT_CAPTURE: &str = "analysis/claude-parallel-subagents";
...
pub fn reconstruct(dir: &Path) -> Result<Vec<RollupsRow>> {
...
fn row_from(path: &Path, base_ms: i64, seq: i64, v: &Value, marks: &FrameMarks) -> RollupsRow {
    ...
    RollupsRow {
        ...
        turn_id: marks.turn_id.as_deref().map(|t| format!("r{base_ms}-{t}")),
