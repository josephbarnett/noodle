...
## Post-057 feature stories (OTel GenAI trace lineage)

057 is the latest **ADR**. The delivery slices that implemented and proved
ADR 052 + 057 carry their own numbers in `docs/features/`; tracked here for
visibility (same glyph legend).

| # | Story | Status | Anchor / notes |
|---|---|---|---|
| 058 | Implement ADR 052: stateless capture + server-side turn/frame correlation | ◐ | Header-driven §5 frame identity (`x-claude-code-agent-id`) + turn segmentation live in `crates/noodle-adapters/src/marking/frame_tree.rs` (#9); corpus oracle `tests/adr_052_frame_tree.rs`. The standalone stateless `record.rs` extractor + full server-side §6 correlation are **not wired** — the live detector still carries per-session turn state in memory. record.rs-vs-frame_tree keeper decision deferred; mid-stream recovery is [063]. |
| 059 | OODA viewer works end-to-end on the new detector marks | ✓ | Per-RT token usage on the OODA HEADERS row (`HeadersBlock`/`thread.ts`, #17); turns/sub-agents/side-call lane verified **live on `:9092`** against real parallel-sub-agent traffic. |
| 060 | Turn-grouped hierarchical GenAI trace spans in the shipper | ✓ | `invoke_agent` frame spans + parented `chat` leaves, side-calls off-tree (#12/#13); corpus regression test `crates/noodle-trace-emitter/tests/corpus_span_tree.rs` (#16). |
| 061 | Dev observability harness + offline trace emitter | ✓ | `docker/otel-genai/` (collector→Tempo→Grafana) + `crates/noodle-trace-emitter` + runbook `docs/operations/otel-genai-harness.md` (#15). |
| 062 | Live GenAI traces end-to-end (proxy → Tempo → TraceQL) | ✓ | Proven live in a `rancher-desktop` cluster: live session → proxy → embellish → shipper → collector → Tempo, navigable `turn → frame → chat` tree (evidence dated in the 061 runbook). |
| 063 | Mid-stream attach: turn recovery from persisted marks | □ | [`063-mid-stream-attach-turn-recovery.md`]. Designed only. Seeds turn state from the SQLite marks store (`ai_telemetry_v_0_0_2`, indexed) behind an ADR 050 `MarkingStateStore` port; one `stop_reason`/`turn_open` ADD COLUMN. Fixes restart/late-attach orphaned turns. First concrete slice of ADR 050. |
...
- **OTel GenAI trace-export chain (052 §10 / 056 / 057 / stories 060–062)** — **proven end-to-end against the real product.** Live agent session → proxy (frame_tree detector) → embellish → shipper (`local-hier`, hierarchical spans) → collector → Tempo → navigable turn→frame→chat tree with `gen_ai.*` + `context.*` attributes. Viewer-side follow-ons all shipped: story 059 (per-RT usage in OODA, #17, live-confirmed), story 057 (left-panel turn-tree navigator, #18), and the story-060 corpus regression test (#16). Dev viewing harness (`docker/otel-genai/`, offline emitter) shipped; prod collector stays ADR 044's separate repo. Only remaining thread in this lineage is feature 063 (mid-stream-attach turn recovery — designed, unbuilt).
